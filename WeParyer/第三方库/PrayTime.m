...
-(id) init {
    ...
    if(self){
        ...
        [timeNames addObject:NSLocalizedStringFromTable(@"key_fajr", @"MyPrayLocalized", @"")];
        [timeNames addObject:@"Sunrise"];
        [timeNames addObject:NSLocalizedStringFromTable(@"key_dhuhr", @"MyPrayLocalized", @"")];
        [timeNames addObject:NSLocalizedStringFromTable(@"key_asr", @"MyPrayLocalized", @"")];
        [timeNames addObject:@"Sunset"];
        [timeNames addObject:NSLocalizedStringFromTable(@"key_maghrib", @"MyPrayLocalized", @"")];
        [timeNames addObject:NSLocalizedStringFromTable(@"key_ishaa", @"MyPrayLocalized", @"")];
...
-(NSMutableArray*)prayerTimes: (NSInteger)interval andLatitude:(double)latitude andLongitude:(double)longitude andtimeZone:(int)tZone calcMethod:(int)methodID {
    //时间间隔
    NSTimeInterval timeInterval = 24*60*60*interval;
    //时间
    ...
    //24小时
    [self setTimeFormat:(int)[self Time24]];
    if (methodID >= 0) {
        [self setCalcMethod:(int)methodID];
    }
    else{
        [self setCalcMethod:(int)[self MWL]];
    }
    [self setHighLatsMethod:3];
    
    NSMutableArray *array = [self getPrayerTimes:dateComponents andLatitude:latitude andLongitude:longitude andtimeZone:[self getTimeZone] + tZone];
    return array;
}

-(NSURL*)shareToWhatapp
{
    NSString* str = (NSString *)CFBridgingRelease(CFURLCreateStringByAddingPercentEscapes(NULL,
                                                                                          (CFStringRef)@"https://itunes.apple.com/us/app/wepray/id1111239523?l=zh&ls=1&mt=8",
                                                                                          NULL,
                                                                                          CFSTR("!*'();:@&=+$,/?%#[]"),
...
//    NSLog(@"%@->%@",CURR_LANG,cur_Lan);
    if([curSelectLan isEqual:@""])
    {
        if (![cur_Lan isEqual:@"en"] && ![cur_Lan isEqual:@"zh"] && ![cur_Lan isEqual:@"ar"] && ![cur_Lan isEqual:@"ru"]) {
    ...
    {
        NSString * path = [[NSBundle mainBundle] pathForResource:curSelectLan ofType:@"lproj"];
        NSBundle * languageBundle = [NSBundle bundleWithPath:path];
        s = [languageBundle localizedStringForKey:translation_key value:@"" table:@"MyPrayLocalized"];
    }
    return s;

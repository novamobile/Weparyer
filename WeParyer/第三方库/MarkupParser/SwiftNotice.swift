...
class SwiftNotice: NSObject {
    ...
    static func showNoticeWithText(type: NoticeType,text: String, autoClear: Bool, autoClearTime: Int) {
        var viewWidth : CGFloat = 90
        if (IS_EN){
            viewWidth = 120
        }
        
        let frame = CGRectMake(0, 0, viewWidth, 90)
        let window = UIWindow()
        window.backgroundColor = UIColor.clearColor()
        let mainView = UIView()
        ...
        checkmarkView.frame = CGRectMake((viewWidth-36)/2, 15, 36, 36)
        mainView.addSubview(checkmarkView)
        
        let label = UILabel(frame: CGRectMake(0, 60, viewWidth, 16))
        label.font = UIFont.systemFontOfSize(13)
        label.textColor = UIColor.whiteColor()
        label.text = text
        label.textAlignment = NSTextAlignment.Center

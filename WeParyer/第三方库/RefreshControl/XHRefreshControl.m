...
- (void)resetScrollViewContentInset {
    ...
    contentInset.bottom = kXHLoadMoreViewHeight + 120;
...
- (void)setScrollViewContentInsetForLoadMore {
    if (self.pullDownRefreshing)
        return;
    UIEdgeInsets currentInsets = self.scrollView.contentInset;
    currentInsets.bottom = kXHLoadMoreViewHeight + 120;
    [self setScrollViewContentInset:currentInsets];
}
...
- (XHLoadMoreView *)loadMoreView {
    if (!_loadMoreView) {
        _loadMoreView = [[XHLoadMoreView alloc] initWithFrame:CGRectMake(0, self.scrollView.contentSize.height-kXHLoadMoreViewHeight, CGRectGetWidth([[UIScreen mainScreen] bounds]), kXHLoadMoreViewHeight)];
